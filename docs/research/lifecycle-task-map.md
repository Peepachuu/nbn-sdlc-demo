...
The running example throughout is the workshop exercise, a fault reporting tool for NBN technicians, because every reader of this document has seen it. Where the example names a tool (GitHub, Claude Code, a CI pipeline), it is the tool used in the workshop or in this team's own repo, not a claim about NBN's stack.
