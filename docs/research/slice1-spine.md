...
Mechanics. Neither source specifies AI ideation, so these are the human collaboration mechanics that sit at this stage. They are templates and checklists rather than commands; the sources offer nothing more specific.
...
Microsoft verdict: AMBER confirmed. The playbook's testing section (unit, integration, E2E, a TDD example, fault injection) is human-authored practice. DORA's continuous-delivery capability requires that effective test suites are "reliable", which it defines as "tests find real failures and only pass releasable code." CI branch protection ("only allow pull request merges when all tests have passed") is the automated half; the human still judges test adequacy.
...
- From Zafir's source note, Liu et al. ([arXiv 2603.28592](https://arxiv.org/abs/2603.28592); 304,362 AI-authored commits): 24.2% of AI-introduced issues were still present at repository HEAD, security issues the most likely to survive (41.1%). The debt outlives the human-approved merge, so migration toward green needs post-merge monitoring as well as a stronger gate.
