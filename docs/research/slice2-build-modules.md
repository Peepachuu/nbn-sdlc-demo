...
The `pre-commit` hook in the same file runs four things on staged files before the message is even asked for: `scripts/validate-placeholders.js` (fails if any `{{placeholder}}` from the boilerplate template survived), Prettier, and ESLint separately for `frontend/` and `backend/`. Prettier is allowed to fail open (`|| true`); the placeholder check and ESLint are not.
...
3. Branch protection. Documented as on. Not verifiable from the repository; it is a GitHub settings page. Check that "Require a pull request before merging" and the CI status checks are set, and whether administrators are included.
